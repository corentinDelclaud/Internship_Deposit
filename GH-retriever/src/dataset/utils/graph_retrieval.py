...
def get_triplets(subgraph):
    """
    Fonction corrigée pour gérer les dimensions dynamiques et les edge_attr None
    """
    # Obtenir les dimensions réelles des features
    node_dim = subgraph.x.shape[1]  # Dimension des node features
    
    # Vérifier si edge_attr existe et n'est pas None
    if subgraph.edge_attr is None or subgraph.edge_attr.numel() == 0:
        # Si pas d'attributs d'arêtes, retourner seulement les node features
        return subgraph.x
    
    edge_dim = subgraph.edge_attr.shape[1]  # Dimension des edge attributes
    
    # S'assurer que les dimensions correspondent
    if node_dim != edge_dim:
        # Ajuster edge_attr pour qu'il ait la même dimension que node features
        if edge_dim < node_dim:
            # Padding avec des zéros
            padding = torch.zeros(subgraph.edge_attr.shape[0], node_dim - edge_dim)
            subgraph.edge_attr = torch.cat([subgraph.edge_attr, padding], dim=1)
        else:
            # Truncation
            subgraph.edge_attr = subgraph.edge_attr[:, :node_dim]
    
    # Maintenant, redimensionner edge_attr pour qu'il ait la même forme que node features
    combined_attr = torch.cat([
        subgraph.x, 
        subgraph.edge_attr.view(-1, node_dim)  # Utiliser node_dim au lieu de 1024
    ], dim=0)
...
def retrive_on_graphs(graph, q_emb, textual_nodes, textual_edges, topk=10, k = 2, topk_entity = 5, augment = "none", sims = None):
    ...
    if augment != "none":  
        ...
        # CORRECTION: Gestion des tailles différentes pour torch.stack
        if len(subg_embs) == 0:
            # Si aucun embedding, créer un tensor vide
            subg_embs = torch.empty(0, q_emb.shape[0], device=q_emb.device)
        else:
            # Trouver la dimension maximale dans la première dimension
            max_dim = max(emb.shape[0] for emb in subg_embs)
            
            # Normaliser tous les tenseurs à la même taille
            normalized_embs = []
            for emb in subg_embs:
                if emb.shape[0] < max_dim:
                    # Padding avec des zéros
                    padding_size = max_dim - emb.shape[0]
                    padding = torch.zeros(padding_size, emb.shape[1], device=emb.device, dtype=emb.dtype)
                    padded_emb = torch.cat([emb, padding], dim=0)
                    normalized_embs.append(padded_emb)
                elif emb.shape[0] > max_dim:
                    # Truncation (garder les premiers max_dim éléments)
                    truncated_emb = emb[:max_dim]
                    normalized_embs.append(truncated_emb)
                else:
                    # Taille correcte
                    normalized_embs.append(emb)
            
            # Maintenant on peut empiler
            subg_embs = torch.stack(normalized_embs)
    
    # Vérifier que subg_embs n'est pas vide avant de calculer la similarité
    if subg_embs.numel() == 0:
        # Si pas d'embeddings, retourner des résultats vides
        empty_graph = Data(
            x=torch.empty(0, graph.x.shape[1], device=graph.x.device, dtype=graph.x.dtype),
            edge_index=torch.empty(2, 0, dtype=torch.long, device=graph.edge_index.device),
            edge_attr=torch.empty(0, graph.edge_attr.shape[1], device=graph.edge_attr.device, dtype=graph.edge_attr.dtype) if graph.edge_attr is not None else None,
            num_nodes=0
        )
        return torch.zeros(0, dtype=q_emb.dtype, device=q_emb.device), (empty_graph, "")
    
    # Calculer la similarité
    # Pour gérer les dimensions différentes, utiliser une approche de pooling
    if subg_embs.dim() == 3:  # [num_subgraphs, max_nodes, feature_dim]
        # Utiliser le pooling moyen pour réduire à [num_subgraphs, feature_dim]
        subg_embs_pooled = subg_embs.mean(dim=1)
    else:
        subg_embs_pooled = subg_embs
    
    sims = torch.nn.CosineSimilarity(dim=-1)(q_emb, subg_embs_pooled)
